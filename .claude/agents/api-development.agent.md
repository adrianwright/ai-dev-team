...
- `docs/reqs/*.html` — The mockups show what data each screen displays, what filters/sorts exist, what actions the user can take, and what fields appear on create/edit forms. **Every API endpoint should trace back to a UI need.**
...
- Use `AstraTerraDbContext` registered as scoped.
...
│   ├── EventsController.cs
│   └── CommunicationPlansController.cs
├── Services/
│   ├── ICaseService.cs
│   ├── CaseService.cs
│   └── ...
